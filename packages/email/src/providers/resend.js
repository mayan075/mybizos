...
export class ResendProvider {
     ...
     */
    async sendEmail(from, to, subject, htmlBody, textBody, tag) {
        const payload = {
    ...
    async sendBatch(messages) {
        ...
        const results = (data?.data ?? []).map((item, idx) => ({
            messageId: item.id,
            submittedAt: new Date().toISOString(),
            to: messages[idx]?.to ?? "",
        }));
     ...
     */
    async send(options) {
        return this.sendEmail(options.from, options.to, options.subject, options.htmlBody, options.textBody, options.tag);
    }
}
 ...
 */
function emailLayout(businessName, preheader, bodyContent, unsubscribeUrl) {
 ...
 */
export function appointmentConfirmationHtml(businessName, contactName, service, date, time, address) {
    const body = `
  ...
  `;
    return emailLayout(businessName, `Your appointment with ${businessName} is confirmed for ${date} at ${time}.`, body);
}
/**
 * Appointment reminder email template (sent 24h before).
 */
export function appointmentReminderHtml(businessName, contactName, service, date, time) {
    const body = `
  ...
  `;
    return emailLayout(businessName, `Reminder: Your ${service} appointment is coming up on ${date} at ${time}.`, body);
}
/**
 * Review request email template (sent after service completion).
 */
export function reviewRequestHtml(businessName, contactName, reviewUrl, feedbackUrl) {
    const body = `
    <h2>How did we do?</h2>
    <p>Hi ${contactName},</p>
  ...
  `;
    return emailLayout(businessName, `${contactName}, how was your experience with ${businessName}? We'd love your feedback.`, body);
}
/**
 * Invoice sent email template.
 */
export function invoiceSentHtml(businessName, contactName, amount, dueDate, paymentLink) {
    const body = `
...
export function welcomeHtml(businessName, userName) {
  ...
  `;
    return emailLayout(businessName, `Welcome to ${businessName}! We're excited to have you as a customer.`, body);
}
