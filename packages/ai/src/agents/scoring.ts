...
export class LeadScoringEngine {
  ...
  score(contact: Pick<Contact, "source" | "phone" | "email">, factors: ScoringFactors): ScoringResult {
    ...
    if (factors.daysSinceLastActivity <= 1) recencyScore = 10;
    else if (factors.daysSinceLastActivity <= 7) recencyScore = 8;
    else if (factors.daysSinceLastActivity <= 30) recencyScore = 5;
    else if (factors.daysSinceLastActivity <= 90) recencyScore = 2;
    else recencyScore = 0;
    ...
    if (finalScore >= AI_SCORE.HOT_LEAD) label = "hot";
    else if (finalScore >= AI_SCORE.WARM_LEAD) label = "warm";
    else label = "cold";
...
Respond with ONLY a JSON object: {"score": number, "reasoning": "brief explanation"}`;
    ...
    const response = await this.client.complete(
      "You are a lead scoring assistant. Respond only with valid JSON.",
      prompt,
      { maxTokens: 200, temperature: 0.3 },
    );

    try {
      const parsed = JSON.parse(response) as { score: number; reasoning: string };
      const aiScore = Math.max(AI_SCORE.MIN, Math.min(AI_SCORE.MAX, parsed.score));

      let label: ScoringResult["label"];
      if (aiScore >= AI_SCORE.HOT_LEAD) label = "hot";
      else if (aiScore >= AI_SCORE.WARM_LEAD) label = "warm";
      else label = "cold";
  ...
  private buildReasoning(label: string, factors: Record<string, number>): string {
    const topFactor = Object.entries(factors)
      .sort(([, a], [, b]) => b - a)[0];

    if (!topFactor) return `Lead rated as ${label}.`;
