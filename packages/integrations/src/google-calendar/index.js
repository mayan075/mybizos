...
export class GoogleCalendarClient {
    ...
    async listEvents(accessToken, calendarId, timeMin, timeMax) {
        ...
        const events = (response.data.items ?? []).map((item) => this.formatEvent(item));
        return {
            events,
     ...
     */
    async updateEvent(accessToken, calendarId, eventId, updates) {
        const calendar = this.getCalendarApi(accessToken);
        const requestBody = {};
    ...
    // ─── Private Helpers ─────────────────────────────────────────────────────
    getCalendarApi(accessToken) {
        const auth = new google.auth.OAuth2(this.config.clientId, this.config.clientSecret, this.config.redirectUri);
