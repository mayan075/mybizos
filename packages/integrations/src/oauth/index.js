 ...
 */
export function buildOAuthUrl(provider, credentials, redirectUri, state) {
 ...
 */
export async function exchangeCodeForTokens(provider, code, credentials, redirectUri) {
    const cfg = OAUTH_PROVIDERS[provider];
    ...
    if (!response.ok) {
        ...
        throw new Error(`OAuth token exchange failed for ${provider}: ${response.status} ${errorText}`);
    }
 ...
 */
export async function refreshAccessToken(provider, refreshToken, credentials) {
    const cfg = OAUTH_PROVIDERS[provider];
 ...
 */
export async function revokeTokens(provider, accessToken, credentials) {
    const cfg = OAUTH_PROVIDERS[provider];
    if (!cfg.revokeUrl)
        return;
    try {
 ...
 */
export async function fetchAccountName(provider, accessToken) {
    try {
        ...
        if (provider === "instagram") {
            const res = await fetch(`https://graph.facebook.com/v19.0/me/accounts?fields=instagram_business_account{username,name}&access_token=${accessToken}`);
            const data = (await res.json());
        ...
        if (provider === "google_business") {
            const res = await fetch("https://mybusinessbusinessinformation.googleapis.com/v1/accounts", { headers: { Authorization: `Bearer ${accessToken}` } });
            const data = (await res.json());
        ...
        if (provider === "google_analytics") {
            const res = await fetch("https://analyticsadmin.googleapis.com/v1beta/accounts", { headers: { Authorization: `Bearer ${accessToken}` } });
            const data = (await res.json());
            const account = data.accounts?.[0];
            return {
